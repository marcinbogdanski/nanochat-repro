...
##############################################
# NOTE: This is copied from PyTorch 2.9.1+cu128
# Changes:
# - relative imports replaced with absolute
# - _to_scalar added here (by Opus 4.5)
# - _disable_dynamo_if_unsupported added here (by Opus 4.5)
# - _params_doc added here (by Opus 4.5)
# - class was renamed to MuonT to avoid name conflict
# Other than that, this file is identical to the PyTorch version.
# This was confirmed by running both PyTorch and this optimized and getting bit-wise identical results
from torch.optim.optimizer import (
    Optimizer,
    ParamsT,
)

# These are private/internal utilities that may not be directly importable
# You may need to implement them or use alternatives:
def _to_scalar(val):
    """Convert a tensor or numeric value to a Python scalar."""
    if isinstance(val, torch.Tensor):
        return val.item()
    return float(val)

def _disable_dynamo_if_unsupported(single_tensor_fn):
    """Decorator that falls back to single tensor implementation."""
    def decorator(func):
        return func
    return decorator

# _params_doc is just a documentation string, define it inline:
_params_doc = "params (iterable): iterable of parameters to optimize or dicts defining parameter groups"
###############################################
